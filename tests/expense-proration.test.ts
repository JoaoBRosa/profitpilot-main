...
describe("expenseAmountForPeriod", () => {
  ...
  it("crédito (valor negativo) reduz o total pontual no dia", () => {
    const amount = expenseAmountForPeriod(
      {
        amountBase: -200,
        frequency: "one-time",
  ...
  it("crédito pontual continua a aparecer via expenseAppliesInPeriod", () => {
    const applies = expenseAppliesInPeriod(
      {
        amountBase: -200,
        frequency: "one-time",
        startDateKey: "2026-03-10",
      },
      { startKey: "2026-03-01", endKey: "2026-03-31" },
    );
